...
export const getParentInfo = (selectedId:number, data:any, parentIds = [], parentTitles = []) => {
  for (const item of data) {
    if (item.id === selectedId) {
      return { ids: [...parentIds, item.id], titles: [...parentTitles, item.title] };
    }
    if (item.childern && item.childern.length > 0) {
      const foundInChild = getParentInfo(selectedId, item.childern, [...parentIds, item.id], [...parentTitles, item.title]);
      if (foundInChild) {
        return foundInChild;
      }
    }
